...
[ Start  ] 	2009.08.
[ Latest ]	2012-01-12 06:29 PM
[ Author ]	김정민
[ Version ]	0.0.0.5
[ Text ]	2레벨 의뢰서가 계속 받아지는 현상 수정
[ Line ]	EP 20
--]]
...
function Ep7_NPCTalkList(iObjIndex)							-- 창고 & 인벤 확장 퀘스트
	...
	local FreeServer = LuaQuestClass:IsFreeServer()						-- 유료: 0,     부분 유료: 1
	local UserLv = 	LuaQuestClass:GetUserLv(iObjIndex)
	local NotGlobalQuest = LuaQuestClass:IsGlobalQuest() 					-- 국내 체크: 0, 글로벌 체크: 1
...
function Ep16_NPCTalkList(iObjIndex)
	...
	ProgEp = ChkProgQuest(Ep16, iObjIndex)-- 진행중인 퀘스트가 존재 하냐?

	if  ProgEp == 1 then
		QS = LuaQuestClass:GetQuestSwitch(Ep16, iObjIndex)
		if QS < 15 then
			LuaQuestClass:AddQuestExpNpcTalk(Ep16, QS)-- NPC한테 보내줄 퀘스트
...
function Ep17_NPCTalkList(iObjIndex)
	...
	ProgEp = ChkProgQuest(Ep17, iObjIndex)-- 진행중인 퀘스트가 존재 하냐?
		
		if  ProgEp == 1 then
			QS = LuaQuestClass:GetQuestSwitch(Ep17, iObjIndex)
			if QS < 15 then
				LuaQuestClass:AddQuestExpNpcTalk(Ep17, QS)-- NPC한테 보내줄 퀘스트
